...
class DeepSeekClient(BaseLLMClient):
    ...
    def chat(
        ...
        if response.usage:
            usage = LLMUsage(
                ...
                cache_read_input_tokens=getattr(response.usage.input_tokens_details, "cached_tokens", 0),
    ...
    def parse_messages(self, messages: list[LLMMessage]) -> ResponseInputParam:
        ...
        for msg in messages:
            if msg.tool_result:
                deepseek_messages.append(self.parse_tool_call_result(msg.tool_result))
            elif msg.tool_call:
                deepseek_messages.append(self.parse_tool_call(msg.tool_call))
            else:
                if not msg.content:
