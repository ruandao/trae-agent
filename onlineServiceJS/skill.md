...
本文档由原 **Python `onlineService`** 的 `skill.md` 全文迁移并改写：路径与字段名与 Python 版对齐，便于任务云与脚本复用；**行为差异**集中在文末「onlineServiceJS 实现差异摘要」与文中加粗标注。
...
| `REPO_ROOT` | 可选。Trae 仓库根目录，默认 **onlineServiceJS 的上一级目录**（与 Python 版「`onlineService` 的上一级」等价）。 |
| `ONLINE_PROJECT_STATE_ROOT` | 可选。运行时状态根目录（其下含 `runtime`、`logs`、`reqLogs` 等），默认 `{REPO_ROOT}/onlineProject_state`。 |
| `TRAE_VENV` | 可选。含 `trae-cli` 的虚拟环境根路径，默认 `{REPO_ROOT}/.venv`；Node 版通过 `{TRAE_VENV}/bin/trae-cli`（及同目录 `python` / `python3` 的 `-m trae_agent.cli`）解析命令。 |
...
容器换票、引导克隆等仍可使用：`TaskApiEndPoint`、`BusinessApiEndPoint`、`BUSINESS_API_ENDPOINT`、`tenantId`、`workspaceId`、`taskId`、`ACCESS_TOKEN`（与任务云约定一致）。**启动就绪日志**：标准输出含 **`[onlineServiceJS] server listening on http://0.0.0.0:<PORT>`**，供编排检测（与 Python Uvicorn 日志不同）。
...
2. 若需真实执行 Trae：确保存在 **`trae-cli`**（`TRAE_VENV` 或 `TRAE_CLI`）；否则 Node 版会对 **`command_kind=trae`** 走**占位 stub**（见下文差异）。
3. `POST /api/repos/clone` 将远程仓库克隆到**新的**可写层（需系统已安装 `git`）。
4. 再 `POST /api/jobs` 创建任务（须满足任务门控；且 Node 版 **必须** 提供 `parent_job_id` 或 `repo_layer_id` 之一，见下文）。
...
**说明**：Node 版**不提供** FastAPI 的 `GET /docs`、`GET /openapi.json`；请以本文档与 `src/server.mjs` 为准。
...
| 方法 | 路径 | 说明 |
|------|------|------|
| `POST` | `/api/repos/clone` | JSON：`url`（必填），`branch`、`depth`、`ssh_identity_file`、`ephemeral_ssh_private_key`、`parent_layer_id`（可选）。在新可写层目录内执行 `git clone`；成功返回 `layer_id`、`layer_path`、`output`。失败时 400，响应体可含 `exit_code`、`detail`。克隆过程与结果可通过 SSE 推送（见下文）。 |
...
- **`parent_layer_id`**（可选）：写入新层 `layer_meta.json` 的父指针；行为与 Python 版一致。
...
| `GET` | `/api/jobs` | 返回 `{ "jobs": [ ... ] }`。字段含 `git_destructive_locked`：**onlineServiceJS 当前恒为 `false`**（未实现 Python 的基线锁定）。 |
| `GET` | `/api/jobs/{job_id}` | 单条任务详情。 |
| `GET` | `/api/jobs/{job_id}/parent` | 父任务：`parent` 为对象或 `null`。 |
| `GET` | `/api/jobs/{job_id}/steps` | 从层目录读取 `.trajectories/trajectory_*.json`（`agent_steps`）或 `.trae_agent_file_log/run_*/step_*`（`agent_step_full.json` 等）；无数据时 `steps` 为空并附 `note`。 |

### `POST /api/jobs` 行为说明（与 Python 对齐部分）

- **前置**：`command_kind=trae` 时需存在有效 `service_config.yaml`；且全局已有至少一层含 `.git`。
- **onlineServiceJS**：**必须且仅能**设置 `parent_job_id` 或 `repo_layer_id` 之一（二者都空或都非空会 400）。**不支持** Python 版「二者皆无则在层根新建空根层再执行」。
- **`parent_job_id`**：从父任务对应层**叠新层**（`createStackedLayer`）；叠层前会按与 Web 串行列表相同的 **created_at 序** 删除**该锚点层之后的全部可写层**（含其任务与目录），再 **purge** 该父层在磁盘上的直接子层（与 Python 意图一致）。
- **`repo_layer_id`**：从指定层叠新层；同样先删串行序中该层**之后**的层，再清理该层的直接子层。
- **`git_branch`**：请求体可携带，**onlineServiceJS 当前未在启动前执行 `git checkout`**（字段预留）。
...
| `POST` | `/api/jobs/{job_id}/redo` | Python 版：删层重建并重跑。**onlineServiceJS：501**，正文含 `detail` 说明。 |
| `POST` | `/api/jobs/{job_id}/continue` | Python 版：中断后继续。**onlineServiceJS：501**。 |
| `DELETE` | `/api/jobs/{job_id}` | 删除任务记录并删除该任务对应可写层目录（实现见 `deleteJob`）。 |
| `POST` | `/api/jobs/reset` | 清空任务、删除已知层目录。**onlineServiceJS 未删除** `commands.json`、`job_events/`、`materialized*` 等（较 Python 版简化）；响应含 `jobs_cleared`、`layers_removed`。 |
...
| `GET` | `/api/layers/{layer_id}/diff/parent` | Python：目录树全文 diff。**onlineServiceJS：未提供该路由**（若需请对照 Python 实现补全）。 |
...
| `POST` | `/api/layers/{layer_id}/queue` | Python：层队列。**onlineServiceJS：未实现该路由**。 |

| 方法 | 路径 | 说明 |
|------|------|------|
| `GET` | `/api/repos/clone-log/{layer_id}` | 克隆过程文本日志（内存缓冲）。 |
| `GET` | `/api/repos/bootstrap-clone-log` | 引导批量克隆日志。 |
| `POST` | `/api/project/view` | JSON：`layer_id`。Python：更新 `onlineProject` 符号链接。**onlineServiceJS：仅返回 JSON `status`/`active_tip_layer_id` 占位**，不保证写 symlink。 |
| `GET` | `/api/project/active` | Python：解析 `onlineProject`。**onlineServiceJS：返回 `bootstrap_layer_id` 与 `note`**（简化）。 |

### 空层锚点（`layer_meta.kind=empty`）

- 服务启动时保证存在 **empty** 锚点目录；**`GET /api/layers` 不列出**这些层。
- 克隆通过 `GET /api/layers/empty-root` + `POST /api/repos/clone` 的 `parent_layer_id` 挂载到锚点（与 Python 文档一致）。

## 层内 Git

| 方法 | 路径 | 说明 |
|------|------|------|
| `GET` | `/api/layers/{layer_id}/git/branches` | Python：列分支。**onlineServiceJS：未实现**。 |
...
| `GET` | `/api/git/identity` | Python：运行时 `user.name` / `user.email`。**onlineServiceJS：固定返回空字符串占位**。 |
| `POST` | `/api/git/identity` | Python：设置身份。**onlineServiceJS：返回 `ok`，未持久化到 git config**（占位）。 |

- **任务云执行日志（git 摘要）**：Python 版在 commit/push 前后可向任务云 `…/git-clone-progress/` 上报。**onlineServiceJS 当前未实现**该上报（克隆引导进度仍以 `bootstrap.mjs` 为准）。
...
常见 `type` 包括：`connected`、`job_created`、`job_started`、`job_output`（含 `chunk`）、`job_finished`、`service_ready` 等；具体以 `broadcast(...)` 调用为准。Python 版部分事件名（如 `repo_clone_finished`）在 Node 中可能略有出入，客户端应兼容未知类型。
...
- **页眉**：展示 **`REPO_ROOT`** 宿主仓库未推送提交数依赖 `GET /api/dev/service-repo-git-push`。**onlineServiceJS 当前为占位响应**（不跑 `git rev-list`）。
- **可写层变动**：依赖 `GET /api/layers/{layer_id}/diff/parent/files` 与 `GET /api/layers/{layer_id}/diff/parent/file?path=…`。Node 版由 `src/layerParentDiff.mjs` 对父层与当前层工作目录做递归条目对比（大目录有条目上限）；无父层时 JSON 带 `detail` 说明。
- 页面通过全局注入的访问令牌调用 API，并用 **`GET /api/events/stream?access_token=…`** 建立 `EventSource`。

### 该页实际调用的 HTTP 接口（与 `dev-local-token` 控制台一致）

以下与 Python 版 `onlineService` 文档同源；若与「实现差异摘要」冲突，**以 Node 实际行为为准**。

| 方法 | 路径 | 用途摘要 |
|------|------|----------|
| `GET` | `/api/dev/service-repo-git-push` | 页眉：宿主仓库未推送提交数；**JS：占位** |
...
| `POST` | `/api/project/view` | JSON：`{"layer_id":"…"}`；**JS：不保证写 symlink** |
| `GET` | `/api/requirements/task-gate` | 是否允许新建任务（`clone_done`）。 |
| `GET` | `/api/layers/empty-root` | 克隆前取空层锚点 `layer_id`。 |
| `POST` | `/api/repos/clone` | 克隆到新建可写层；**JS：见上文 clone 字段差异** |
...
| `GET` | `/api/jobs/{job_id}/steps` | 步骤手风琴数据；**JS：常为空** |
| `GET` | `/api/jobs/{job_id}/parent` | 「查询父任务」调试区。 |
| `POST` | `/api/jobs` | 创建任务；**JS：须 parent 或 repo_layer** |
| `POST` | `/api/jobs/{job_id}/interrupt` | 中断。 |
| `POST` | `/api/jobs/{job_id}/redo` | 重新执行；**JS：501** |
| `POST` | `/api/jobs/{job_id}/continue` | 继续；**JS：501** |
| `DELETE` | `/api/jobs/{job_id}` | 删除任务。 |
| `POST` | `/api/jobs/reset` | 「重置」；**JS：清理范围简化** |
| `GET` | `/api/layers` | 下拉与层图（不含 `empty` 锚点）。 |
| `DELETE` | `/api/layers/{layer_id}` | zTree「删除该层」。 |
| `POST` | `/api/layers/{layer_id}/queue` | 运行中「加入队列」；**JS：未实现** |
...
未在 `index.html` 中直连的接口（如 `GET /api/layers/{layer_id}/files` 仅前缀列表、`GET /api/jobs/{job_id}/events`、`GET /api/layers/{layer_id}/git/branches` 等）在 Python 版可查 OpenAPI；**Node 版无 `/docs`、`/openapi.json`，且上述部分路由未实现**。

## 层命名

`layer_id` 形如 `YYYYMMDD_HHMMSS_xxxxxx`（时间戳 + 6 位十六进制后缀），与 Python 版一致。
...
构建**上下文必须为 `trae-agent` 仓库根目录**（与历史 Python 镜像相同约定）：
...
- 子层叠加通过符号链接共享父层 `.git`（`layerFs.mjs`），与 Python 版设计一致。
- **`git_destructive_locked`**：Python 版用于防误操作；**Node 版未实现**，字段恒为 `false`。
- **`POST /api/jobs/reset`** 会删除层目录，慎用；清理范围见上文（较 Python 可能更少）。
- 配置、克隆日志与任务输出可能含敏感信息，请妥善保管 `ACCESS_TOKEN` 与运行时文件。

## onlineServiceJS 实现差异摘要（速查）
...
| `diff/parent` 全文 | **onlineServiceJS：未提供**；`diff/parent/files` 与 `diff/parent/file` 已实现（见 `layerParentDiff.mjs`）。 |
| `POST .../queue`、`GET .../git/branches`、`GET .../jobs/.../events` | 未实现 |
...
若需与历史 Python 版**完全等价**的行为，请在 Node 实现中补齐上述项；静态控制台资源已收录于 **`onlineServiceJS/static`**，构建镜像不再依赖 `onlineService/` 目录。
